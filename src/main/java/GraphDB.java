...
public class GraphDB {
    /** SABRINA ADDED
     * instance variables to store your graph
     */
    //neighbors: key -> ID of vertex; value -> ID of neighboring vertices
    public HashMap<Long, ArrayList<Long>> neighbors = new HashMap<>();
    //allMapNodes: key -> ID of MapNode; value -> MapNode object corresponding to ID
    public HashMap<Long, MapNode> allMapNodes = new HashMap<>();
     ...
     */
    private void clean() {
        //the neighbors HashMap will hold all nodes with connections
        //remove the nodes in allMapNodes that aren't in neighbors
        ArrayList toRemove = new ArrayList();
        for (Long id : allMapNodes.keySet()) {
            if (!neighbors.keySet().contains(id)) {
                toRemove.add(id);
            }
        }
        allMapNodes.keySet().removeAll(toRemove);
    }
     ...
     */
    double lon(long v) {
        if (!allMapNodes.keySet().contains(v)) {
            return 0.0;
        }
        MapNode nodeV = allMapNodes.get(v);
        return nodeV.getLon();
    }
    ...
    Iterable<Long> vertices() {
        //all vertex IDs in graph will be in keyset of allMapNodes after cleaning
        return allMapNodes.keySet();
    }
    ...
    public long closest(double lon, double lat) {
        MapNode given = new MapNode(0L, lat, lon);
        allMapNodes.put(given.GetID(), given);
        double shortestDistance = 0.0;
        long IDofClosest = 0L;
        for (long currID : allMapNodes.keySet()) {
            double calcDistance = distance(currID, given.GetID());
            if (IDofClosest == 0L) {
                shortestDistance = calcDistance;
                IDofClosest = currID;
            } else if (calcDistance < shortestDistance) {
                shortestDistance = calcDistance;
                IDofClosest = currID;
            }
        }
        allMapNodes.remove(0L, given);
        return IDofClosest;
    }
