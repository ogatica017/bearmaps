...
public class GraphBuildingHandler extends DefaultHandler {
    ...
    //SABRINA - I ADDED THIS
    public boolean isValid = false;
    public ArrayList<Long> possibleConnections = new ArrayList<>();
    ...
    public void startElement(String uri, String localName, String qName, Attributes attributes)
        ...
        if (qName.equals("node")) {
            ...
            MapNode newNode = new MapNode(id, lat, lon);
            lastNode = newNode;
            g.allMapNodes.put(newNode.GetID(), newNode);
            ...
            //SABRINA: the nd should already exist. so should be in allMapNodes already
            //MapNode currNode = g.allMapNodes.get(attributes.getValue("ref"));
            Long currID = Long.parseLong(attributes.getValue("ref"));
            possibleConnections.add(currID);
    ...
    public void endElement(String uri, String localName, String qName) throws SAXException {
        if (qName.equals("way")) {
            ...
            //SABRINA - if the connections are valid, then set up neighbor connections
            if (isValid == true) {
                for (int i = 0; i < possibleConnections.size() - 1; i++) {
                    Long firstID = possibleConnections.get(i);
                    Long secondID = possibleConnections.get(i+1);
                    //add connection between firstID -> secondID
                    if (!g.neighbors.containsKey(firstID)) {
                        ArrayList newSet = new ArrayList();
                        newSet.add(secondID);
                        g.neighbors.put(firstID, newSet);
                    } else {
                        ArrayList oldSet = g.neighbors.get(firstID);
                        oldSet.add(secondID);
                        g.neighbors.put(firstID, oldSet);
                    }
                    //add connection between secondID -> firstID
                    if (!g.neighbors.containsKey(secondID)) {
                        ArrayList newSet = new ArrayList();
                        newSet.add(firstID);
                        g.neighbors.put(secondID, newSet);
                    } else {
                        ArrayList oldSet = g.neighbors.get(secondID);
                        oldSet.add(firstID);
                        g.neighbors.put(secondID, oldSet);
                    }
                }
            }
